...
def aggregate_tests(
    ...
    if not do_group:
        return [(name, time_s, 1) for name, time_s in tests]

    grouped: dict[str, float] = defaultdict(float)
    counts: dict[str, int] = defaultdict(int)

    for full_name, time_s in tests:
        key = group_name(full_name)
        grouped[key] += time_s
        counts[key] += 1

    return [(name, grouped[name], counts[name]) for name in grouped]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Print the longest-running CTest tests from a JUnit XML file "
            "(default: ctest.xml)."
        )
    )
    parser.add_argument(
        ...
        nargs="?",
        default="ctest.xml",
        help="JUnit XML file produced by CTest (default: %(default)s)",
    )
    parser.add_argument(
        "-n",
        "--top",
...
def main() -> int:
    ...
    tests = parse_testcases(xml_path)
