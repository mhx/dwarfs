...
TEST(compact_reader, throws_on_invalid_bool_element_encoding) {
  ...
  EXPECT_THAT(
      [&] { (void)r.read_bool(); },
      ThrowsMessage<tl::protocol_error>(HasSubstr("invalid bool encoding")));
}
...
TEST(compact_reader, read_string_and_binary_enforce_max_string_bytes) {
  ...
  auto const opt = tl::decode_options{.max_struct_depth = 64,
                                      ...
                                      .max_string_bytes = 2};
  auto r1 = tl::compact_reader{
      std::span<std::byte const>{bytes.data(), bytes.size()}, opt};
  auto r2 = tl::compact_reader{
      std::span<std::byte const>{bytes.data(), bytes.size()}, opt};
...
TEST(compact_reader,
  ...
  auto r =
      tl::compact_reader{std::span<std::byte const>{out.data(), out.size()}};
...
TEST(compact_reader, read_double_is_not_supported) {
  ...
  EXPECT_THAT([&] { (void)r.read_double(); },
              ThrowsMessage<tl::protocol_error>(HasSubstr(
