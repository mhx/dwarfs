...
TEST(scoped_output_capture_test, constructor_throws_when_dup_fails) {
  auto ops = test::real_scoped_output_capture_posix_ops();
  ops.dup_fn = [](int) -> int {
    errno = EMFILE;
    return -1;
  };

  test::scoped_output_capture_posix_ops_override ops_guard{ops};
  ...
  EXPECT_THROW(
      {
        try {
          ...
          EXPECT_THAT(e.what(), HasSubstr("saving original fd failed"));
          throw;
        }
      },
      std::system_error);
...
TEST(scoped_output_capture_test, constructor_throws_when_pipe_fails) {
  auto ops = test::real_scoped_output_capture_posix_ops();
  ops.pipe_fn = [](int[2]) -> int {
    errno = ENFILE;
    return -1;
  };
  ...
  EXPECT_THROW(
      {
        try {
          ...
          EXPECT_EQ(e.code().value(), ENFILE);
          EXPECT_THAT(e.what(), HasSubstr("pipe failed"));
          throw;
        }
      },
      std::system_error);
...
TEST(scoped_output_capture_test,
     constructor_rolls_back_when_redirect_dup2_fails) {
  int dup2_calls = 0;

  auto ops = test::real_scoped_output_capture_posix_ops();
  ops.dup2_fn = [&](int, int) -> int {
    ++dup2_calls;
    errno = EBADF;
    return -1;
  };
  ...
  {
    test::scoped_output_capture_posix_ops_override ops_guard{ops};
    EXPECT_THROW(scoped_output_capture{}, std::system_error);
  }

  EXPECT_EQ(2, dup2_calls) << "dup2 should have been called twice";

  // After rollback, a normal capture should still work.
  {
    scoped_output_capture cap;
    std::fputs("ok", stdout);
    EXPECT_EQ(cap.captured(), "ok");
  }
}

TEST(scoped_output_capture_test, stop_rethrows_reader_error) {
  auto ops = test::real_scoped_output_capture_posix_ops();
  ops.read_fn = [](int, void*, std::size_t) -> ssize_t {
    errno = EIO;
    return -1;
  };
  ...
  EXPECT_THROW(
      {
        try {
          ...
          EXPECT_THAT(e.what(), HasSubstr("read failed"));
          throw;
        }
      },
      std::system_error);
...
TEST(scoped_output_capture_test, reader_retries_after_eintr) {
  std::atomic<bool> first_read = true;

  auto ops = test::real_scoped_output_capture_posix_ops();
  ops.read_fn = [&](int fd, void* buf, std::size_t count) -> ssize_t {
    if (first_read.exchange(false)) {
      errno = EINTR;
      return -1;
    }
  ...
  EXPECT_EQ(cap.captured(), "hello");
}

TEST(scoped_output_capture_test, stop_throws_when_restore_dup2_fails) {
  int dup2_call_count = 0;

  auto ops = test::real_scoped_output_capture_posix_ops();
  ops.dup2_fn = [&](int from, int to) -> int {
    ++dup2_call_count;
    if (dup2_call_count == 2) {
      errno = EIO;
      return -1;
    }
    return ::dup2(from, to);
  ...
  EXPECT_THROW(
      {
        try {
          ...
          EXPECT_EQ(e.code().value(), EIO);
          EXPECT_THAT(e.what(), HasSubstr("restoring original stream failed"));
          throw;
        }
      },
      std::system_error);
