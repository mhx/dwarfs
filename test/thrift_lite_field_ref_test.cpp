...
TEST(field_ref, preserves_rvalue_reference_qualifier) {
  ...
  auto ref = tl::field_ref<int&&>{std::move(v)};
  static_assert(std::is_same_v<decltype(ref.value()), int&&>);
...
TEST(optional_field_ref, unset_state_behaves_like_empty_optional) {
  ...
  EXPECT_THAT([&] { (void)ref.value(); }, Throws<std::bad_optional_access>());
  EXPECT_THAT([&] { (void)*ref; }, Throws<std::bad_optional_access>());
  EXPECT_THAT([&] { (void)ref.operator->(); },
...
TEST(optional_field_ref, reset_clears_bit_but_does_not_touch_underlying_value) {
  auto v = 123;
  auto isset = std::bitset<1>{};
  isset.set(0);
