...
TEST(debug_writer, set_end_throws_if_not_all_elements_written) {
  auto oss = std::ostringstream{};
  auto w = tl::debug_writer{oss};
...
TEST(debug_writer, map_end_throws_if_not_all_entries_written) {
  auto oss = std::ostringstream{};
  auto w = tl::debug_writer{oss};
...
TEST(debug_writer, type_errors) {
  auto oss = std::ostringstream{};
  auto w = std::optional<tl::debug_writer>{};
  w.emplace(oss);
  w->write_struct_begin("TestStruct");
  w->write_field_begin("a_double", tl::ttype::double_t, 2);
  EXPECT_THAT([&] { w->write_bool(true); },
              ThrowsMessage<tl::protocol_error>(
                  HasSubstr("expected value of type double but got bool")));
  w.reset();
  oss.str("");
  w.emplace(oss);
  w->write_struct_begin("TestStruct");
  w->write_field_begin("a_uuid", tl::ttype::uuid_t, 3);
  EXPECT_THAT([&] { w->write_string("not a uuid"); },
              ThrowsMessage<tl::protocol_error>(
                  HasSubstr("expected value of type uuid but got string")));
  w.reset();
  oss.str("");
  w.emplace(oss);
  w->write_struct_begin("TestStruct");
  EXPECT_THAT([&] { w->write_field_begin("what?", tl::ttype::stop_t, 3); },
              ThrowsMessage<tl::protocol_error>(
                  HasSubstr("field type cannot be stop")));
  w.reset();
  oss.str("");
  w.emplace(oss);
  w->write_struct_begin("TestStruct");
  EXPECT_DEATH(w->write_field_begin("oops", static_cast<tl::ttype>(100), 3),
               "unknown ttype: 100");
}

TEST(debug_writer, logic_errors) {
  auto oss = std::ostringstream{};
  auto w = std::optional<tl::debug_writer>{};
  w.emplace(oss);
  w->write_struct_begin("TestStruct");
  ...
  EXPECT_THAT([&] { w->write_field_begin("oops", tl::ttype::i32_t, 2); },
              ThrowsMessage<tl::protocol_error>(
                  HasSubstr("previous field value not written")));
}
