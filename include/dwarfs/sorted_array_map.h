...
class sorted_array_map {
 public:
  ...
  constexpr mapped_type operator[](key_type const& k) const { return at(k); }

  constexpr mapped_type at(key_type const& k) const {
  ...
  constexpr const_reverse_iterator crbegin() const { return data_.crbegin(); }
