...
class block_compressor {
 public:
  ...
  shared_byte_buffer compress(shared_byte_buffer data) const {
    return impl_->compress(std::move(data), nullptr);
  }

  shared_byte_buffer
  compress(shared_byte_buffer data, std::string const& metadata) const {
  ...
  class impl {
   public:
    ...
    compress(shared_byte_buffer data, std::string const* metadata) const = 0;
