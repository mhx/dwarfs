...
case "-$BUILD_TYPE-" in
  *-small-*)
    CMAKE_ARGS="${CMAKE_ARGS} -DENABLE_PERFMON=0 -DENABLE_DEBUG_LOGGING=0 -DTRY_ENABLE_BROTLI=0"
    CMAKE_ARGS="${CMAKE_ARGS} -DDISABLE_CXA_DEMANGLE=1 -DTRY_ENABLE_LIBMAGIC=0"
    ;;
  *-minimal-*)
    CMAKE_ARGS="${CMAKE_ARGS} -DENABLE_PERFMON=0 -DENABLE_DEBUG_LOGGING=0 -DWITH_MAN_OPTION=0"
    CMAKE_ARGS="${CMAKE_ARGS} -DTRY_ENABLE_BROTLI=0 -DTRY_ENABLE_LZ4=0 -DTRY_ENABLE_FLAC=0"
    CMAKE_ARGS="${CMAKE_ARGS} -DENABLE_RICEPP=0 -DDISABLE_FILESYSTEM_EXTRACTOR_FORMAT=1"
    CMAKE_ARGS="${CMAKE_ARGS} -DDISABLE_CXA_DEMANGLE=1 TRY_ENABLE_LIBMAGIC=0"
    ;;
  *-dev-*)
...
case "-$BUILD_TYPE-" in
  *-asan-*|*-tsan-*|*-ubsan-*|*-coverage-*)
    CMAKE_ARGS="${CMAKE_ARGS} -DWITH_UNIVERSAL_BINARY=1 -DWITH_FUSE_EXTRACT_BINARY=1 -DWITH_LEGACY_FUSE=1"
    ;;
esac
