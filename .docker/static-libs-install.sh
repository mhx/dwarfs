...
if use_lib mimalloc; then
    ...
    mkdir build
    cd build
    cmake .. -DMI_LIBC_MUSL=ON -DMI_BUILD_SHARED=OFF -DMI_BUILD_OBJECT=OFF -DMI_BUILD_TESTS=OFF -DMI_OPT_ARCH=OFF -DMI_INSTALL_TOPLEVEL=ON -DCMAKE_INSTALL_PREFIX="$INSTALL_DIR" ${CMAKE_ARGS}
    $NINJA_PARALLEL
    ninja install
fi
