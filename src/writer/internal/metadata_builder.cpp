...
thrift::metadata::metadata const& metadata_builder_<LoggerPolicy>::build() {
  ...
  // TODO: when upgrading, we can use a lower time resolution,
  //       but we can't recover any previously lost precision,
  //       so increasing time resolution should not be allowed
  ...
  if (options_.pack_shared_files_table) {
    if (!md_.shared_files_table()->empty()) {
      ...
      DWARFS_CHECK(std::is_sorted(sf.begin(), sf.end()),
  ...
  //       => time resolution should not be changed automatically
  md_.options() = fsopts;
  md_.features() = features_.get();

  // TODO: try and keep metadata upgrade history
  // TODO: the create timestamp should *never* be changed, only
  //       remove; it should also never be set after the fact
  auto dwarfs_version(std::string("libdwarfs ") + DWARFS_GIT_ID);

  if (is_rewrite_) {
    md_.rebuild_dwarfs_versions().ensure().push_back(dwarfs_version);

    if (options_.no_create_timestamp) {
      md_.create_timestamp().reset();
    }
  } else {
    md_.dwarfs_version() = dwarfs_version;
...
void metadata_builder_<LoggerPolicy>::upgrade_metadata() {
  ...
  if (md_.entry_table_v2_2().is_set()) {
